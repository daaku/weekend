...
class ContainerContext(object):
  ...
  def _send_rest_request(self, request):
    ...
    js = self._handle_response(http_response)
    return request.process_json(js)
  ...
  def _send_rpc_requests(self, batch):
    ...
    for response in js:
      id = response.get('id')
      js = response.get('data')
      key = id_to_key_map[id]
      request = batch.requests[key]
      batch._set_data(key, request.process_json(js))
  ...
  def _handle_response(self, http_response):
    ...
    if http_response.status == httplib.OK:
      js = json.loads(http_response.content)
